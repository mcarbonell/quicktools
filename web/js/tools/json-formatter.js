...
prettyBtn?.addEventListener('click', () => {
    try {
        ...
        showMsg('JSON formateado.', 'success');
    } catch (e) {
        showMsg('JSON inválido: ' + e.message, 'error');
    }
});
...
minifyBtn?.addEventListener('click', () => {
    try {
        ...
        showMsg('JSON minificado.', 'success');
    } catch (e) {
        showMsg('JSON inválido: ' + e.message, 'error');
    }
});
...
copyBtn?.addEventListener('click', async () => {
    try {
        if (jsonInput.value.trim() === '') {
            showMsg('No hay texto para copiar.', 'error');
            return;
        }
        ...
        try {
            ...
            showMsg('Error al copiar: ' + e.message + '\n' + (err?.message || ''), 'error');
        }
    }
});
